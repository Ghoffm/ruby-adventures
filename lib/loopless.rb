...
module GoingLoopless
  ...
  def list_movies(person)
    person.roles.uniq { |role| role.movie }.map {|element| element.movie}.sort_by {|movie| movie.year}.map {|movie| "#{movie.title} (#{movie.year})" }
  end
  ...
  movie.roles.select { |role| role_order.include?(role.name) }.sort_by{ |role| role_order.index(role.name) }.map { |element| "#{element.person.name} (#{element.name})"}.to_set
  end
end
